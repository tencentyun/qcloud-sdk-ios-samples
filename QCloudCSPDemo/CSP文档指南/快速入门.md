...
|   |         |DELETE | DELETE Bucket cors  | 完全支持|
|   |acl（访问权限控制列表）         |PUT| PUT Bucket  acl |完全支持|
|   |         |GET | GET Bucket  acl | 完全支持|
| Object（对象） | Object（对象）        |PUT| PUT Object |完全支持|
|   |         |PUT | PUT Object - Copy   | 完全支持(大小不能超过5MB)|
|   |         |GET | GET Object   | 完全支持|
|   |         |DELETE | DELETE Object   | 完全支持|
|   |         |DELETE | DELETE Multiple Objects   | 部分支持：DELETE Multiple Object 接口请求实现在指定 Bucket 中批量删除 Object，单次请求最大支持批量删除 1000 个 Object|
|   |         |HEAD | HEAD Object   | 完全支持|
|   |         |OPTIONS | OPTIONS Object   | 完全支持|
|   |acl（访问权限控制列表）         |PUT| PUT Object  acl |完全支持|
|   |         |GET | GET Object  acl | 部分支持：GET Object acl 接口用来获取某个存储桶下的某个对象的访问权限，只有存储桶的持有者才有权限操作。|
